...
SDL_Texture *loadFont(SDL_Renderer *renderer, int colorgrade)
{
    ...
    for (int y = 0; y < FONTHEIGHT; y++)
    {
        for (int x = 0; x < FONTWIDTH; x++)
        {
            ...
            if (colorgrade == 0)
            {
                color = font[y][x] ? white : black;
            }
            else
            {
                color = font[y][x] ? black : white;
            }
...
void draw_char(SDL_Renderer *renderer, unsigned char character, int cx, int cy, int scale, SDL_Texture *fontTexture)
{
...
void draw_text(SDL_Renderer *renderer, const char *text, int x, int y, int scale, SDL_Texture *fontTexture)
{
